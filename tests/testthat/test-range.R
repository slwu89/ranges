...
test_that("create_range_log correctly makes ranges", {

  # basic range generation
  expect_equal(create_range_log(lo = 0, hi = 1024, mult = 2), c(0, 2^(0:10)))
  expect_equal(create_range_log(lo = -1024, hi = 1024, mult = 2), c(-2^(10:0), 0, 2^(0:10)))
  expect_equal(create_range_log(lo = -1024, hi = 1024, mult = 3), c(-1024, -3^(6:0), 0, 3^(0:6), 1024))
  expect_equal(create_range_log(lo = 0, hi = 0), 0)
  expect_equal(create_range_log(lo = 0,hi = 1), c(0, 1))
  expect_equal(create_range_log(lo = 10, hi = 10), 10)

  # things are floor-d
  expect_equal(
    create_range_log(lo = -1e3, hi = -10, mult = 10),
    create_range_log(lo = -1e3, hi = -10, mult = 10.9)
  )
})

test_that("create_range_log errors for bad input", {
  # bad mult
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), hi = -8, mult = -1))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), mult = 1))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), mult = 0))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), mult = Inf))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), mult = -Inf))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), mult = NULL))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), mult = NA))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), mult = NaN))

  # bad lo
  expect_error(create_range_log(lo = -Inf, hi = -8, mult = 2))
  expect_error(create_range_log(lo = Inf, hi = -8, mult = 2))
  expect_error(create_range_log(lo = NULL, hi = -8, mult = 2))
  expect_error(create_range_log(lo = NA, hi = -8, mult = 2))
  expect_error(create_range_log(lo = NaN, hi = -8, mult = 2))

  # bad hi
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), hi = Inf, mult = 2))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), hi = -Inf, mult = 2))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), hi = NULL, mult = 2))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), hi = NaN, mult = 2))
  expect_error(create_range_log(lo = -bitwShiftL(a = 8, n = 10), hi = NA, mult = 2))

  # bad range
  expect_error(create_range_log(lo = 8, hi = 2, mult = 2))
  expect_error(create_range_log(lo = -2, hi = -64, mult = 2))

  # out of integer range (error from stopifnot(hi >= lo), warning from as.integer(2^31))
  create_range_log(lo = -(2^31), hi = -2, mult = 2) %>%
    expect_error() %>%
    expect_warning()
})
